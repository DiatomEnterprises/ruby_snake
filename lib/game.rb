...
class Game
  ...
  def print_board
    ...
    puts "Your opponent's size is: #{snake2.size} "
    puts "---------------------------"
  ...
  def check_if_snake_met_wall
    ...
    snake2.update_head(1,0) if snake2.head[1] > gameboard.width-1
    snake2.update_head(1, gameboard.width-1) if snake2.head[1] < 0
    snake2.update_head(0, 0) if snake2.head[0]  > gameboard.length-1
    snake2.update_head(0, gameboard.length-1) if snake2.head[0] < 0
  end
  ...
  def check_if_snake_ate_food
    ...
    if snake2.head[0] == food.x && snake2.head[1] == food.y
      snake2.increase
      @food = Food.new(gameboard.width, gameboard.length)
    end
  end
  ...
  def execute_action key
    ...
    snake.turn(key)
    snake2.turn(['q','w','s','d'][Random.rand(4)])
  end
